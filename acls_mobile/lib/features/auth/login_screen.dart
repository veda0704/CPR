...
class _LoginScreenState extends ConsumerState<LoginScreen> {
                ...
                children: [
                          ...
                          child: DecoratedBox(
                            ...
                            child: ElevatedButton(
                              ...
                              child: authState.status == AuthStatus.loading
                                  ...
                                  : Text(isTe ? 'లాగిన్' : 'LOGIN', style: GoogleFonts.inter(fontWeight: FontWeight.w800, fontSize: 17, letterSpacing: 1.5, color: Colors.white)),
                            ),
                          ),
                        ),
                      ),
                  ...
                  Text.rich(
                    TextSpan(
                      ...
                      children: [WidgetSpan(child: GestureDetector(onTap: () => context.push('/signup'), child: Text(isTe ? 'సైన్ అప్' : 'Sign Up', style: GoogleFonts.inter(color: AppColors.orange, fontWeight: FontWeight.w800))))],
                    ),
                  ),
  ...
  Widget _buildForm(AuthState state, bool isTe) {
    return Column(
      children: [
        _buildInput(controller: _emailCtrl, hint: isTe ? 'ఈమెయిల్' : 'Email Address', icon: Icons.email_outlined),
        ...
        Row(
          ...
          children: [
            Row(
              children: [
                SizedBox(width: 24, height: 24, child: Checkbox(value: true, onChanged: (v) {}, activeColor: AppColors.orange)),
                const SizedBox(width: 8),
                Text(isTe ? 'నన్ను గుర్తుంచుకోండి' : 'Remember me', style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.darkOrange)),
              ],
            ),
            Text(isTe ? 'పాస్‌వర్డ్ మర్చిపోయారా?' : 'Forgot Password?', style: GoogleFonts.inter(fontSize: 14, fontStyle: FontStyle.italic, fontWeight: FontWeight.w600, color: AppColors.darkOrange.withValues(alpha: 0.7))),
          ],
        ),
        if (state.status == AuthStatus.error && state.errorMessage != null)
          Padding(padding: const EdgeInsets.only(top: 16), child: Text(state.errorMessage!, textAlign: TextAlign.center, style: GoogleFonts.inter(color: AppColors.red, fontSize: 13, fontWeight: FontWeight.w600))),
      ],
    );
  }

  Widget _buildInput({required TextEditingController controller, required String hint, required IconData icon, bool isPassword = false, bool obscure = false, VoidCallback? onToggleObscure}) {
    return Container(
      height: 56,
      ...
      child: Row(
        children: [
          Container(width: 56, height: 56, color: AppColors.darkOrange, child: Icon(icon, color: Colors.white, size: 22)),
          Expanded(child: TextField(controller: controller, obscureText: obscure, style: GoogleFonts.inter(fontSize: 16, color: AppColors.text, fontWeight: FontWeight.w500), decoration: InputDecoration(hintText: hint, hintStyle: GoogleFonts.inter(color: AppColors.muted, fontSize: 15), border: InputBorder.none, contentPadding: const EdgeInsets.symmetric(horizontal: 16), suffixIcon: isPassword ? IconButton(icon: Icon(obscure ? Icons.visibility_outlined : Icons.visibility_off_outlined, color: AppColors.muted, size: 20), onPressed: onToggleObscure) : null))),
        ],
      ),
    );
  }
